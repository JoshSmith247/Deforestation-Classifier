...
![Deforestation](https://ecologi.com/resources/blog/industries-and-sectors-driving-deforestation-what-you-need-to-know)
...
The figures below were generated by running the full pipeline on a region in Pará, Brazil (coordinates `[-55.5, -7.6, -55.4, -7.5]`). Sentinel-2 cloud-free composites were pulled from Google Earth Engine for each year from 2016–2023, and the trained U-Net predicted forest coverage for each image. Deforestation is reported as the percentage of pixels classified as non-forest.

Figures are produced by [`_plot_trend`](src/amazon_analysis.py) and [`_plot_maps`](src/amazon_analysis.py) in `src/amazon_analysis.py`, called from the [`analyze()`](src/amazon_analysis.py) pipeline function.

### Deforestation trend (2016–2023)

![Deforestation trend](blog_figures/amazon_deforestation_trend.png)

### Per-year satellite image and forest mask

![Deforestation maps](blog_figures/amazon_deforestation_maps.png)

## Output

- `blog_figures/amazon_deforestation_maps.png` — Side-by-side satellite image and predicted forest mask for each year
- `blog_figures/amazon_deforestation_trend.png` — Line chart of deforestation percentage over time
